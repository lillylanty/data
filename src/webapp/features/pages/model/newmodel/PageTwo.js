...
class ShowEntityForm extends Component {
  ...
  dynamicFormItem = ()=>{
    const {entityModelAttr} = this.props;
    let arr = [];
    ...
    if(entityModelAttr.length <1){
      return
    }
     
      entityModelAttr.forEach(e => {
        switch(e.attrName){
          case '编码':
            arr.push(
              <FormItem
  ...
  render() {
    ...
    const {editEntityModelAttr ,entityModelAttr,modalData,saveEntity,alldata } = this.props;
    //动态展示表单
